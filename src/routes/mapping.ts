...
/**
 * POST /api/mapping/teams/batch-delete
 * 批量删除映射
 *
 * Body:
 * {
 ...
 * }
 */
router.post('/teams/batch-delete', async (req: Request, res: Response) => {
  try {
    const { ids } = req.body;
    ...
    logger.info(`[API] 批量删除 ${ids.length} 条球队映射`);

    const results = await mappingManager.batchDelete(ids);
    ...
    logger.error('[API] 批量删除映射失败:', error.message);
    res.status(500).json({
      success: false,
      error: error.message,
    });
